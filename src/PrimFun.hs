...
module PrimFun (diffprimfun) where
...
diffprimfun :: (Elt a, Elt b, IsFloating b)
            => Tools (ScalarType a) (ScalarType b) -> PrimFun (a -> b) -> Exp a -> Exp b -> Exp b
...
class DifferentiatePrimFunFF a b valid | a b -> valid where
  diffAllPrimFunsFF :: Tools (ScalarType a) (ScalarType b) -> PrimFun (a -> b) -> Exp a -> Exp b -> Exp b
...
class DifferentiatePrimFunTFF t atup b valid | atup b -> valid where
  diffAllPrimFunsTFF :: t -> PrimFun (atup -> b) -> Exp atup -> Exp b -> Exp b

instance {-# OVERLAPPING #-} (Differentiate a a) => DifferentiatePrimFunTFF (Tools a a) (a, a) a HTrue where
  diffAllPrimFunsTFF = diffprimfunTFF

instance {-# OVERLAPPING #-} (r ~ HFalse) => DifferentiatePrimFunTFF t a b r where
...
diffprimfunFF tk (PrimExpFloating ty) a dx = chainUnary a a dx
...
diffprimFunOF :: (IsFloating tb, Elt tb, Eq tb,
              ...
              => Tools ta tb -> PrimFun (ta -> tb) -> Exp tb -> Exp tb -> Exp tb
